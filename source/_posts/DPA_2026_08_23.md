---
title: "Accuracy Rivals Mainstream Models, Speed Surpasses Empirical Potentials: DPA4 OMat24 and OMol25 Pre-trained Models Unveiled"
date: 2026-08-23
categories:
- DPA
---

## Accuracy on Par with Mainstream Models, Speed Surpassing Empirical Potentials; DPA4 OMat24 and OMol25 Pre-trained Models Debuted Simultaneously

> Source: https://mp.weixin.qq.com/s/HHGTI9AACzlG9BIMBmHCNA
> 
> ⚠️ Formulas have been automatically extracted; manual proofreading is recommended
> ⚠️ Audio and video content has been skipped (see the end of the article for details)
...
## Usage Tutorial (Bohrium + MatMaster)

### 1. Submitting Calculations via Web or Bohrium CLI

- Web Interface (Recommended for Beginners)

You can directly use the Bohrium web interface (https://www.bohrium.com/job-commit) to submit tasks: create a new container task, select the project <project-id> and the desired GPU machine type, choose the image registry.dp.tech/dptech/deepmd-kit:3.2.0-sm89, upload the task dependency files, fill in the startup command, and submit. The startup commands for various DPA4 / DPA4C use cases are summarized below:

```bash
# DPA4: Training / Fine-tuning / Freezing / Evaluation / Compression
dp --pt train input.json
dp --pt train finetune.json --finetune pretrained.pt
dp --pt freeze -c model.ckpt.pt -o frozen_model
dp --pt test -m frozen_model.pt2 -s data/system -n 100

# DPA4C: Change backend to pt-expt, use graph lower for freeze
dp --pt-expt train input.json
dp --pt-expt train finetune.json --finetune pretrained.pt
dp --pt-expt freeze -c model.ckpt.pt -o frozen_model --lower-kind graph
dp --pt-expt test -m frozen_model.pt2 -s data/system -n 100
dp --pt-expt compress -i frozen_model.pt2 -o compressed_model.pt2
# Single GPU LAMMPS/Kokkos MD
lmp -k on g 1 -sf kk -in in.lammps
# Multi-GPU: One MPI rank per GPU
CUDA_VISIBLE_DEVICES=0,1,2,3 mpirun -np 4 lmp -in in.lammps
```

In the LAMMPS input, you must write `atom_modify map yes` and ensure that the element order in `pair_coeff * *` exactly matches the model's `type_map` and LAMMPS atom types; it is recommended to set the neighbor skin to a non-zero value (e.g., `neighbor 2.0 bin`) to avoid rebuilding the neighbor list at every step.

Note for DPA4 users: `DP_TRITON_INFER` must be exported before `dp --pt freeze`—the exported `.pt2` is an AOTInductor artifact, and the inference strategy will be solidified into the computational graph, so it will not be re-read when loaded by LAMMPS later.

<center><img src=https://dp-public.oss-cn-beijing.aliyuncs.com/community/Blog%20Files/DPA_2026_08_23/pic19.png pic_center width="100%" height="100%" /></center>

- bohr-cli

You can also submit via the command line using `bohr job` or `bohr batchjob`. Taking `bohr job` as an example: write the above commands into an execution script as needed (e.g., `run.sh`), and place the DeePMD input, dataset, pre-trained model, and LAMMPS input into the case folder <case-dir>.

```bash
bohr job submit \
  -m "registry.dp.tech/dptech/deepmd-kit:3.2.0-sm89" \
  -t "<machine-type>" \
  -c "bash run.sh" --input_directory "<case-dir>" \
  --project_id "<project-id>" -n "<job-name>" \
  --max_run_time "<minutes>"
```

To check tasks, use `bohr job list -n 10`, `bohr job describe -j <job-id> --json`, and `bohr job log -j <job-id>`. For formal tasks, it is recommended to submit using `bohr batchjob`; see the bohr-cli manual (https://doc.bohrium.com/docs/bohrctl/about/) for details.

### 2. Completing the Full DPA4/DPA4C Workflow on MatMaster Using Natural Language

The materials science AI agent MatMaster (https://matmaster.bohrium.com/matmaster/chat) allows you to bypass environment configuration and lengthy command lines: it has built-in images and recommended GPU machine types for DPA4/DPA4C, supports uploading input files, models, and data, and only requires you to describe the scientific problem in natural language. It will break down the requirements into specific task steps and execute them. We are currently using MatMaster + DPA4 to reproduce key potential calculation results from the literature, and the case collection will be released successively.

<center><img src=https://dp-public.oss-cn-beijing.aliyuncs.com/community/Blog%20Files/DPA_2026_08_23/pic20.png pic_center width="100%" height="100%" /></center>
## Related Links
...
<center><img src=https://mmbiz.qpic.cn/mmbiz_jpg/uDiaxE3FhLbtMcHYDGksl9kjuB2c03JQEdWO8DflKROzqM5nnaDgfRL8iaAI1cyNdOn8yKaibATqst0NcSdXHico89Dg8EPKecL8iauiap2WlD9Ig/640?wx_fmt=jpeg&from=appmsg pic_center width="100%" height="100%" /></center>

Add the administrator's WeChat to join the DPA4 WeChat early-access group for discussion and exchange.